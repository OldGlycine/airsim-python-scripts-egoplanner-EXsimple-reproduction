...
class EgoPlanner:
    def __init__(self, drone_name="Drone1", lidar_sensors=["LidarSensor1"]):
        # 初始化AirSim客户端
    ...
    def take_control(self):
        ...
        print(f"已获取 {self.drone_name} 的控制权")
    ...
    def release_control(self):
        ...
        print(f"已释放 {self.drone_name} 的控制权")
    
    def get_drone_pose(self):
        """
        获取无人机位姿
        返回值：position, orientation, linear_velocity, augular_velocity
        """
    ...
    def get_drone_state(self):
        """获取无人机状态 返回值：gps_location,timestamp,landed_state,rc_data,ready,ready_message"""
        drone_state = self.client.getMultirotorState(vehicle_name=self.drone_name)
        return {
    ...
    def get_lidar_data(self, sensor_name):
        """获取雷达数据 返回：points,time_stamp,pose"""
        try:
    ...
    def check_segment_collision(self, start, end):
        ...
        if length == 0:
            # return self.check_collision(start)
            return False
    ...
    def find_pv_pairs(self, control_point, path):
        ...
        if closest_point is not None and min_dist < self.safety_distance * 2:
            ...
            if draw:
                # ① 绘制closest_point（红色点）
                try:
                    # 将点转换为AirSim Vector3r对象
                    point_vec = airsim.Vector3r(closest_point[0], closest_point[1], closest_point[2])
                    # 绘制红色点，持续时间0.1秒
                    self.client.simPlotPoints(
                        [point_vec], 
    ...
    def safe_move_to_position(self, x, y, z, velocity, min_distance=0.5):
        ...
        if distance < min_distance:
            if distance < 0.1:  # 非常接近目标
                print(f"Already very close to target ({distance:.2f}m), maintaining hover")
            ...
            print(f"Short distance ({distance:.2f}m), using velocity control for {move_time:.2f}s")
        ...
        while self.is_running:
            try:
                ...
                if distance_to_goal < 1.0:
                    ...
                    print(f'目前距离{distance_to_goal:.2f}m')
    ...
    def takeoff(self, altitude=5.0):
        ...
        self.client.moveToZAsync(-altitude, 2.0, vehicle_name=self.drone_name).join()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 设置目标位置
        start_position = planner.get_drone_pose()['position']
        goal_position =  np.array([25.799999237060547, 0.19999998807907104, -11.380000114440918 ])
        
        # goal_points = find_target_actors('WayPoints.*')
        # print(goal_points)
        # cur_position = start_position
        # for goal_point in goal_points:
            # goal_position = np.array([goal_point['x'], goal_point['y'], goal_point['z']])
            # print(f"开始规划从 {cur_position} 到 {goal_point['name']}:{goal_position}")
            # planner.plan_to_position(goal_position)
            # print(f"已抵达{goal_point['name']},进入下一次导航")
        planner.plan_to_position(goal_position)
        print('导航成功')
        # 等待到达目标或手动停止
